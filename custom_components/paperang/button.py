...
class PaperangPrintButton(PaperangEntity, ButtonEntity):
    ...
    async def async_press(self) -> None:
        ...
        if (state := hass.states.get(prefix)) is not None:
            mode = state.state
        if (
            state := hass.states.get("text.paperang_p2_printer_print_content")
        ) is not None:
            content = state.state or ""
        if (
            state := hass.states.get("number.paperang_p2_printer_font_size")
        ) is not None:
            font_size = int(float(state.state))
        if (
            state := hass.states.get("number.paperang_p2_printer_heat_density")
        ) is not None:
        ...
        if (
            state := hass.states.get("select.paperang_p2_printer_image_profile")
        ) is not None:
            profile = state.state
...
class PaperangFeedButton(PaperangEntity, ButtonEntity):
    ...
    async def async_press(self) -> None:
        ...
        lines = 50
        if (
            state := self.hass.states.get("number.paperang_p2_printer_feed_lines")
        ) is not None:
...
class PaperangTestPrintButton(PaperangEntity, ButtonEntity):
    ...
    def __init__(self, coordinator) -> None:
        """Initialize."""
        super().__init__(
            coordinator, "Test Print", "paperang_p2_btn_test_print", "mdi:printer-check"
        )
