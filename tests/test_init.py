...
class TestGetPrinter:
    ...
    def test_usb_with_bus_port_uses_custom_transport(self):
        ...
        with (
            patch.object(mod, "UsbTransportWithPath") as mock_tp,
    ...
    def test_ble_without_address(self):
        import custom_components.paperang as mod
        mod._transport_configs[FAKE_ENTRY_ID] = {"transport": "ble"}
    ...
    def test_ble_not_installed_falls_back_to_usb(self):
        ...
        result = mod._get_printer(FAKE_ENTRY_ID)
...
class TestUsbTransportWithPath:
    ...
    @pytest.mark.skip(reason="requires USB hardware or proper usb.core mock")
    def test_connect_raises_when_device_not_found(self):
        from custom_components.paperang import UsbTransportWithPath

        transport = UsbTransportWithPath(bus=99, port=[99], vid=0x4348, pid=0x5584)
        with pytest.raises(RuntimeError, match="not found at bus=99 port=\\[99\\]"):
            transport.connect()
